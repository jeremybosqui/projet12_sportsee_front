...
import { Container } from "../Style/Performance_User";
 ...
 */
 ...
 function UserPerformance() {
    ...
    useEffect(() => {
		const data = async () => {
			const [request] = await Promise.all([getData("USER_PERFORMANCE", id)])
			if (!request) {
			...
			const formatData = request["data"].data.map((data) => {
				...
				switch (data.kind) {
					...
					case 2: return { ...data, kind: 'Energie' }
					case 3: return { ...data, kind: 'Endurance' }
					case 4: return { ...data, kind: 'Force' }
					case 5: return { ...data, kind: 'Vitesse' }
					case 6: return { ...data, kind: 'Intensité' }
					default: return {...data }
				}
			})
			setData(formatData)
